...
from ..schemas import HabitCreate, ProfileUpdate, SearchResult, SkillCreate, SkillUpdate
...
def list_skills(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[dict]:
    ...
    return [{"id": item.id, "name": item.name, "progress": item.progress, "tone": item.tone, "category": item.category} for item in skills]
...
def create_skill(payload: SkillCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    record_activity(db, current_user.id)
    db.commit()
    return {"id": skill.id, "name": skill.name, "progress": skill.progress, "tone": skill.tone, "category": skill.category}
...
def list_habits(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[dict]:
    ...
    return [{"id": h.id, "label": h.label, "complete": h.complete, "streak": h.streak, "tone": h.tone} for h in habits]
...
def create_habit(payload: HabitCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    record_activity(db, current_user.id)
    db.commit()
    return {"id": habit.id, "label": habit.label, "complete": habit.complete, "streak": habit.streak, "tone": habit.tone}
...
def notifications(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[dict]:
    items = db.scalars(select(Notification).where(Notification.user_id == current_user.id).order_by(Notification.created_at.desc()).limit(50)).all()
    return [{"id": item.id, "message": item.message, "kind": item.kind, "is_read": item.is_read, "created_at": item.created_at} for item in items]


@router.post("/notifications/read-all")
def read_all_notifications(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    items = db.scalars(select(Notification).where(Notification.user_id == current_user.id, Notification.is_read.is_(False))).all()
    for item in items:
        item.is_read = True
    db.commit()
