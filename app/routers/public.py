...
def public_profile(username: str, db: Session = Depends(get_db)) -> dict:
    ...
    return {
        ...
        "skills": [{"id": item.id, "name": item.name, "progress": item.progress, "tone": item.tone} for item in skills],
...
def public_members(
    ...
    for user in users:
        ...
        members.append({
            **user_summary(user),
            "top_skills": [{"name": skill.name, "tone": skill.tone} for skill in top_skills],
            "current_project": (
