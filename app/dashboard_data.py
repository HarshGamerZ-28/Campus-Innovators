...
def build_dashboard(db: Session, user: User, private: bool = True) -> dict:
    ...
    return {
        ...
        "habits": [
            {"id": item.id, "label": item.label, "complete": item.complete, "streak": item.streak, "tone": item.tone}
            for item in habits
        ],
        "projects": [project_public(project, user.id if private else None) for project in projects if private or project.is_public],
        "achievements": [
            {"label": "Problem Solver", "tone": "purple", "icon": "award"},
            {"label": "Streak Master", "tone": "orange", "icon": "flame"},
            {"label": "Early Bird", "tone": "gold", "icon": "sparkles"},
            {"label": "Project Builder", "tone": "blue", "icon": "hammer"},
        ],
